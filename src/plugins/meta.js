...
function updateClient () {
  if (ssrTakeover) {
    ssrTakeover = false
    this.$root.__currentMeta = window.__U_META__
...
function getServerMeta (app, html) {
  ...
  const tokens = {
    '%%U_HTML_ATTRS%%': Object.keys(meta.htmlAttr)
      .filter(htmlFilter)
      .map(getAttr(meta.htmlAttr))
      .join(' '),
    '%%U_HEAD_TAGS%%': getHead(meta),
    '%%U_BODY_ATTRS%%': Object.keys(meta.bodyAttr)
      .filter(bodyFilter)
      .map(getAttr(meta.bodyAttr))
      .join(' '),
    '%%U_BODY_TAGS%%': Object.keys(meta.noscript)
      .map(name => `<noscript data-qmeta="${name}">${meta.noscript[name]}</noscript>`)
      .join('') +
      `<script data-qmeta-init>window.__U_META__=${delete meta.noscript && JSON.stringify(meta)}</script>`
  }
  ...
  install ({ queues, Vue }) {
    if (isSSR) {
      ...
      queues.server.push((q, ctx) => {
        ctx.ssr.U_HTML_ATTRS += ' %%U_HTML_ATTRS%%'
        Object.assign(ctx.ssr, {
          U_HEAD_TAGS: '%%U_HEAD_TAGS%%',
          U_BODY_ATTRS: '%%U_BODY_ATTRS%%',
          U_BODY_TAGS: '%%U_BODY_TAGS%%'
        })
      })
    }
    else {
